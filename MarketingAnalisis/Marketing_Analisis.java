...
public class Marketing_Analisis extends Configured implements Tool {
      ...
      public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException{
        ...
        try{
          for(int i=0;i<tuple.length; i++){
            ...
            context.write(new Text(tweet_id),new Text(tweet_text+"\t----->\t"+new Text(Integer.toString(Marketing_sum))));
          }
        } catch(Exception e) {
        e.printStackTrace();
        }
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new Marketing_Analisis(),args);
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    DistributedCache.addCacheFile(new URI("/dictionary/AFINN.txt"),conf);
    Job job = new Job(conf, "MarketingAnalysis");
    job.setJarByClass(Marketing_Analisis .class);
